        ...
        case 'CHANGE-TASK-STATUS': {
            ...
            return {
                ...state, [action.todolistId]: state[action.todolistId].map(
                    t => t.id === action.taskId ? {...t, status: action.status} : t
                )
            }
        }
...
//в дз переписать кейсы и экшен чтобы один thunk creator изменял и статус и title
// export const updateTaskTC = ((todoId: string, taskId: string, status: TaskStatuses) => (dispatch: Dispatch,
...
export const updateTaskTC = ((todoId: string, taskId: string, parameter: TaskStatuses | string) => (dispatch: Dispatch,
                                                                                        getState: () => AppRootStateType) => {
    const task = getState().tasks[todoId].find((el) => el.id === taskId)

    if (task) {
        const model: UpdateTaskModelType = {
            title: typeof parameter === 'string' ? parameter : task.title,
            description: task.description,
            status: typeof parameter !== 'string' ? parameter : task.status,
            priority: task.priority,
            startDate: task.startDate,
            deadline: task.deadline
        }
        ...
        todolistsAPI.updateTask(todoId, taskId, model).then((res) => {
            typeof parameter === 'string'
                ? dispatch(changeTaskTitleAC(taskId, parameter, todoId))
                : dispatch(changeTaskStatusAC(taskId, parameter, todoId))
        })
    }
})
